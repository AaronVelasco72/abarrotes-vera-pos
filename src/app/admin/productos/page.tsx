...
import { CATEGORIES, CATEGORY_LIST, CategoryBadge, CategoryChip, type CategoryKey } from "@/lib/categories";
...
function ProductModal({ product, onClose }: { product: Product | null; onClose: () => void }) {
  ...
  async function onSave() {
    ...
    // Doc ya está guardado. Si hay imagen, intenta subir aparte para no bloquear.
    if (file && savedId) {
      setPhase("uploading");
      try {
        const fb = getFirebase();
        ...
        setErr(
          "Producto guardado, pero la imagen no subió: " + (e as Error).message,
        );
        setPhase("idle");
        return;
      }
      ...
      <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-3xl shadow-2xl max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sticky bottom-0 bg-white border-t border-brand-50 p-4 grid grid-cols-3 gap-2">
          ...
          <button onClick={onSave} disabled={busy} className={`btn-primary ${isEdit ? "col-span-1" : "col-span-2"}`}>
            {phase === "saving" ? "Guardando…" : phase === "uploading" ? "Subiendo imagen…" : "Guardar"}
          </button>
        </div>
      </div>
    </div>
